...
namespace PD.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (admins.Result.Count() == 0 && _configuration.GetValue<bool>("AutoSetFirstAdmin"))
                        {
                            ...
                            if (firstUser != null)
                            {
                                Task task = _userManager.AddToRoleAsync(firstUser, "Admin");
                                task.Wait();

                                if (task.IsCompletedSuccessfully)
                                {
                                    _logger.LogInformation("Granted Admin role to " + firstUser.Email);

                                    //Reloading roles of the current user
                                    await _signInManager.SignInAsync(user, false);
                                }
                                else
                                    _logger.LogError("Failed to assign Admin role to default admin user.");
                            }
